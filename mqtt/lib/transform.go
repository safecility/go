...
type PahoTransformer interface {
	...
	TransformPahoJoinMessage(m paho.Message) (*messages.SimpleMessage, error)
	TransformPahoUplinkMessage(m paho.Message) (*messages.SimpleMessage, error)
	TransformPahoDownlinkMessage(m paho.Message, channel messages.MqttChannel) (*messages.SimpleMessage, error)
	CreateDownlink(message messages.SimpleMessage, correlationIDs []string) ([]byte, error)
}
...
type PayloadAdjuster interface {
	AdjustPayload(m *messages.SimpleMessage) error
}

type IdentityAdjuster struct{}

func (i IdentityAdjuster) AdjustPayload(_ *messages.SimpleMessage) error {
	return nil
}
